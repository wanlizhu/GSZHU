...
namespace GSZHU {
    ...
    public:
        CTextureFormat(ETexture type, EPixelFormat format, uint32_t width, uint32_t height)
            : mType(type)
            , mWidth(width)
            , mHeight(height)
            , mPixelFormat(format)
        ...
        ETexture GetTextureType() const { return mType; }
