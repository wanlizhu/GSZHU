...
namespace Wanlix
{
    struct BoundingBox
    {
        ...
        static BoundingBox FromMinMax(Float3 const& min, Float3 const& max) {
            BoundingBox box;
        ...
        static BoundingBox Union(BoundingBox const& box0, BoundingBox const& box1) {
            return BoundingBox::FromMinMax(glm::min(box0.GetMinPos(), box1.GetMinPos()),
                                           glm::max(box0.GetMaxPos(), box1.GetMaxPos()));
        }

        FORCEINLINE bool operator==(BoundingBox const& rhs) const {
        ...
        FORCEINLINE BoundingBox transform(Float4x4 const& mat) const {
