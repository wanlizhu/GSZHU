...
#define UNEXPECTED(msg, ...) ASSERTION_FAILED(msg, ##__VA_ARGS__)

#define CHECK_ERROR(expr, msg, ...) \
do { \
    if (!(expr)) { ASSERTION_FAILED(msg, ##__VA_ARGS__); } \
} while (false)
#else
#define ASSERTION_FAILED(...) do{} while(false)
#define UNEXPECTED(msg, ...) do{} while(false)
#define CHECK_ERROR(...) do{} while(false)
#endif
...
namespace Wanlix
{
    ...
    using ByteArrayPtr = std::shared_ptr<Array<Uint8>>;
    using StringArray = Array<String>;
    using StringArrayPtr = std::shared_ptr<Array<String>>;
    ...
    struct Range
    {
        ...
        {
            return std::hash<Uint>{}(offset) ^ (std::hash<Uint>{}(size) << 1);
        }
    ...
    struct Extent
    {
        ...
        inline Uint64 Hash() const
        {
            return std::hash<Uint>{}(width) ^ (std::hash<Uint>{}(height) << 1);
        }
    ...
    struct Offset
    {
        ...
        inline Uint64 Hash() const
        {
            return std::hash<Uint>{}(x) ^ (std::hash<Uint>{}(y) << 1);
        }
    ...
    struct Region
    {
        ...
        inline Uint64 Hash() const
        {
            return offset.Hash() ^ (extent.Hash() << 1)
        }
