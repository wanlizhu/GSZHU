...
namespace Wanlix
{
    thread_local static std::vector<Display::UniquePtr>* g_displayListRef = nullptr;
    ...
    )
    {
        if (g_displayListRef)
        {
            g_displayListRef->push_back(Win32Display::Create(monitor));
            return TRUE;
        }
        return FALSE;
    }
    ...
    INT2 Win32Display::GetOffset() const
    {
        MONITORINFO info;
        GetInfo(info);
        return INT2(info.rcMonitor.left, 
                      info.rcMonitor.top);
    }
    ...
    bool Win32Display::SetDisplayMode(const DisplayMode& displayModeDesc)
    {
        // Get display device name
        MONITORINFOEX infoEx;
        GetInfo(infoEx);
    ...
    std::vector<DisplayMode> Win32Display::GetSupportedDisplayMode() const
    {
        std::vector<DisplayMode> displayModeDescs;
        ...
        SortDisplayModes(displayModeDescs);
