...
namespace ZHU
{
	...
	public:
		...
		public:
			SharedPtr() = default;
			SharedPtr(VkResource<BufferType, ImageType>* res) 
				: std::shared_ptr<VkResource<BufferType, ImageType>>(res)
			{}

			static SharedPtr Create(BufferType buffer, VkDeviceMemory mem) 
			{ 
				return SharedPtr(new VkResource(buffer, mem));
			}
			static SharedPtr Create(ImageType image, VkDeviceMemory mem) 
			{ 
				return SharedPtr(new VkResource(image, mem));
			}
			VkResourceType GetResourceType() const 
			{ 
				return get()->mType;
			}
			operator BufferType() const 
			{
				assert(get()->mType == VkResourceType::Buffer);
				return get()->mBufferHandle;
			}
			operator ImageType() const 
			{ 
				assert(get()->mType == VkResourceType::Image);
				return get()->mImageHandle;
			}
			operator VkDeviceMemory() const 
			{ 
				return get()->mDeviceMemory; 
			}
		};
	...
	protected:
		VkResource(BufferType image, VkDeviceMemory mem) 
			: mType(VkResourceType::Buffer)
			, mBufferHandle(image)
			, mDeviceMemory(mem)
		{}
		VkResource(ImageType buffer, VkDeviceMemory mem) 
			: mType(VkResourceType::Image)
			, mImageHandle(buffer)
			, mDeviceMemory(mem) 
		{}
