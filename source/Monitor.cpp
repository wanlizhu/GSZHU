#include "Wanlix/Utils/Monitor.h"
...
{
    #ifndef _WIN32
    void Minitor::GetMinitorDescs(Array<Minitor>& descs) {}
    void Minitor::PrintMinitorDesc() {}
    #else
    ...
    std::vector<MonitorDesc> internalDescs;
                                  ...
                                  LPARAM   dwData)
    {
        ...
        if (GetMonitorInfo(hMonitor, &info))
        {
            ...
            MonitorDesc desc;
            desc.identifier = ToString(DeviceID.c_str());
            desc.resolution = glm::vec2(
                abs(info.rcMonitor.left - info.rcMonitor.right),
                abs(info.rcMonitor.top - info.rcMonitor.bottom));

            //printf("%fx%f mm\n", WidthMm, HeightMm );
            desc.size = glm::vec2(wInch, hInch);
            auto vPpi = desc.resolution / desc.size;
    ...
    void Monitor::GetMonitorDescs(Array<MonitorDesc>& monitorDescs)
    {
        internalDescs.clear();
    ...
    void Monitor::PrintMonitorDesc()
    {
        std::vector<MonitorDesc> monitorDescs;
        GetMonitorDescs(monitorDescs);

        for (auto& desc : monitorDescs)
        {
            printf("%s%s: %d x %d pix, %d x %d in, %0.2f ppi\n",
