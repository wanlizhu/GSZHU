...
int zip_entry_close(struct zip_t *zip) {
    ...
    if (level) {
        ...
        zip->entry.AddMethod = MZ_DEFLATED;
    }
    ...
    if (!mz_zip_writer_create_local_dir_header(
            ...
            zip->entry.comp_size, zip->entry.uncomp_crc32, zip->entry.AddMethod, 0,
            dos_time, dos_date)) {
    ...
    if (!mz_zip_writer_add_to_central_dir(
            ...
            zip->entry.uncomp_crc32, zip->entry.AddMethod, 0, dos_time, dos_date,
