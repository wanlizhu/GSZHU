...
#include "Vulkan/DrawingDevice_Vulkan.h"
...
void DrawingSystem::FlushEntity(std::shared_ptr<IEntity> pEntity)
{
    ...
    else
    {
        if (!m_bClearTarget)
        {
			assert(m_pCameraList.size() > 0);
            m_pDevice->ClearTarget(m_pContext->GetSwapChain(), m_pCameraList[0]->GetComponent<CameraComponent>()->GetBackground());
            m_bClearTarget = true;
        }
        ...
        if (pEntity->HasComponent<PolylineRendererComponent>())
        {
            ...
            if (pRenderer != nullptr)
                pRenderer->AttachSegments(pEntity->GetComponent<PolylineRendererComponent>()->GetGeometry(), *m_pResourceTable);
        }
        if (pEntity->HasComponent<EllipseArcRendererComponent>())
        {
            auto& pRenderer = std::dynamic_pointer_cast<EllipseArcRenderer>(gpGlobal->GetRenderer(eRenderer_EllipseArc));
            if (pRenderer != nullptr)
                pRenderer->AttachEllipseArcs(pEntity->GetComponent<EllipseArcRendererComponent>()->GetGeometry(), *m_pResourceTable);
        }
    }
}
...
bool DrawingSystem::CreateDevice()
{
    ...
    {
        case eDevice_D3D11:
            m_pDevice = CreateNativeDevice<EConfigurationDeviceType::eDevice_D3D11>();
            break;
        case eDevice_D3D12:
            m_pDevice = CreateNativeDevice<EConfigurationDeviceType::eDevice_D3D12>();
            break;
		case eDevice_Vulkan:
			m_pDevice = CreateNativeDevice<EConfigurationDeviceType::eDevice_Vulkan>();
			break;
        default:
            assert(false);
    }
...
void DrawingSystem::BuildFrameGraph()
{
    for (auto& pCamera : m_pCameraList)
    {
        ...
        BuildAllFrameGraph(pFrameGraph, pCamera);
    }
}

bool DrawingSystem::BuildAllFrameGraph(std::shared_ptr<FrameGraph> pFrameGraph, std::shared_ptr<IEntity> pCamera)
{
	BuildForwardFrameGraph(pFrameGraph, pCamera);
	BuildPolylineFrameGraph(pFrameGraph, pCamera);
	BuildEllipseArcFrameGraph(pFrameGraph, pCamera);
...
bool DrawingSystem::BuildPolylineFrameGraph(std::shared_ptr<FrameGraph> pFrameGraph, std::shared_ptr<IEntity> pCamera)
{
	auto& pRenderer = gpGlobal->GetRenderer(eRenderer_Polyline);
	if (pRenderer == nullptr)
		return false;

	pRenderer->CreateDataResources(*m_pResourceTable);

	auto pMainPass = pRenderer->GetPass(PolylineRenderer::LineSegmentPass());
	assert(pMainPass != nullptr);
...
bool DrawingSystem::BuildEllipseArcFrameGraph(std::shared_ptr<FrameGraph> pFrameGraph, std::shared_ptr<IEntity> pCamera)
{
	auto& pRenderer = gpGlobal->GetRenderer(eRenderer_EllipseArc);
	if (pRenderer == nullptr)
		return false;

	pRenderer->CreateDataResources(*m_pResourceTable);

	auto pMainPass = pRenderer->GetPass(EllipseArcRenderer::EllipseArcPass());
	assert(pMainPass != nullptr);
