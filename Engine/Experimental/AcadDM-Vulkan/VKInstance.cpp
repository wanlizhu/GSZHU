...
namespace AutoCAD::Graphics::Engine
{
    ...
    bool VKInstance::IsLayerSupported(const char* name)
    {
    ...
    bool VKInstance::IsExtensionSupported(const char* name)
    {
    ...
    VKInstance::~VKInstance()
    {
        if (IsValid())
        {
            if (mDebugLayerType == EDebugLayerType::Messenger)
            {
                vkDestroyDebugUtilsMessengerEXT(mInstance, mDebugLayer.messenger, nullptr);
                mDebugLayer.messenger = VK_NULL_HANDLE;
            }
            else if (mDebugLayerType == EDebugLayerType::ReportCallback)
            {
                vkDestroyDebugReportCallbackEXT(mInstance, mDebugLayer.reportCallback, nullptr);
                mDebugLayer.reportCallback = VK_NULL_HANDLE;
            }
    ...
    bool VKInstance::IsValid() const
    {
        return mInstance != VK_NULL_HANDLE;
    }

    bool VKInstance::IsValidationLayerEnabled() const
    {
        return mDebugLayerType != EDebugLayerType::None;
    }

    bool VKInstance::IsLayerEnabled(const char* name) const
    {
        return std::find_if(
    ...
    bool VKInstance::IsExtensionEnabled(const char* name) const
    {
        return std::find_if(
    ...
    uint32_t VKInstance::GetPhysicalDeviceCount() const
    {
        uint32_t count = 0;
    ...
    VkPhysicalDevice VKInstance::FirstPhysicalDevice() const
    {
    ...
    VkPhysicalDevice VKInstance::FindPhysicalDeviceBy(
        ...
        , mEnabledExtensions(enabledExtensions)
    {
        ...
        {
            if (VKInstance::IsExtensionSupported(VK_EXT_DEBUG_UTILS_EXTENSION_NAME))
            {
                ...
                {
                    mDebugLayerType = EDebugLayerType::Messenger;
                    ...
                    VK_CHECK(vkCreateDebugUtilsMessengerEXT(mInstance, &createInfo, nullptr, &mDebugLayer.messenger));
                }
            }
            else if (VKInstance::IsExtensionSupported(VK_EXT_DEBUG_REPORT_EXTENSION_NAME))
            {
                vkCreateDebugReportCallbackEXT = LOAD_INSTANCE_FUNC(mInstance, vkCreateDebugReportCallbackEXT);
                vkDestroyDebugReportCallbackEXT = LOAD_INSTANCE_FUNC(mInstance, vkDestroyDebugReportCallbackEXT);
                if (vkCreateDebugReportCallbackEXT == nullptr || vkDestroyDebugReportCallbackEXT == nullptr)
                {
                    LOG_ERROR("vk*DebugReportCallbackEXT functions are missing, did you forget to enable '%s' extension?",
                        VK_EXT_DEBUG_REPORT_EXTENSION_NAME);
                }
                else
                {
                    mDebugLayerType = EDebugLayerType::ReportCallback;
                    ...
                    VK_CHECK(vkCreateDebugReportCallbackEXT(mInstance, &createInfo, nullptr, &mDebugLayer.reportCallback));
                }
            }
            else
            {
                mDebugLayerType = EDebugLayerType::None;
            }
        }
        else
        {
    ...
    VKInstanceBuilder& VKInstanceBuilder::SetAppName(const char* name)
    {
        mAppInfo.pApplicationName = name;
        return *this;
    }

    VKInstanceBuilder& VKInstanceBuilder::SetEngineName(const char* name)
    {
        mAppInfo.pEngineName = name;
        return *this;
    }

    VKInstanceBuilder& VKInstanceBuilder::SetApiVersion(uint32_t version)
    {
        mAppInfo.apiVersion = version;
        return *this;
    }

    VKInstanceBuilder& VKInstanceBuilder::EnableValidationLayers(bool value)
    {
        mEnableValidationLayers = value;
        return *this;
    }

    VKInstanceBuilder& VKInstanceBuilder::EnableLayer(const char* name, bool compulsory)
    {
        if (VKInstance::IsLayerSupported(name))
        {
    ...
    VKInstanceBuilder& VKInstanceBuilder::EnableExtension(const char* name, bool compulsory)
    {
        if (VKInstance::IsExtensionSupported(name))
        {
    ...
    SharedPtr<VKInstance> VKInstanceBuilder::Build()
    {
        ...
        auto raw = new VKInstance(mCreateInfo, mEnabledLayers, mEnabledExtensions);
        return SharedPtr<VKInstance>(raw);
    }
}