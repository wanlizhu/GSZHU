...
namespace AutoCAD::Graphics::Engine
{
    ...
    {
        assert(mCreateInfo.surface != VK_NULL_HANDLE);
        VkSurfaceCapabilitiesKHR surfaceCapabilities = {};
        VK_CHECK(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(mDevice->GetPhysicalDevice(), mCreateInfo.surface, &surfaceCapabilities));
    ...
    {
        auto raw = new GISwapchainVk(mDevice, mAcquireNextImageAsync, mComponentMapping, mCreateInfo);
        assert(raw->IsValid());
        return SharedPtr<GISwapchainVk>(raw);
    }
}