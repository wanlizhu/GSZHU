...
namespace AutoCAD::Graphics::Engine
{
        ...
        , mPhysicalDevice(physicalDevice)
    {
        ...
        {
            const auto& queueFamilies = mPhysicalDeviceInfo.supportedQueueFamilies;
            uint32_t familyIndex = createInfo.pQueueCreateInfos[i].queueFamilyIndex;
            VkQueue queue = VK_NULL_HANDLE;
            vkGetDeviceQueue(mLogicalDevice, familyIndex, 0, &queue);

            if (VK_QUEUE_GRAPHICS_BIT & queueFamilies[familyIndex].queueFlags)
            {
                mGraphicsQueue.reset(new GIDeviceQueueVk(weak_from_this(), queue, familyIndex));
            }

            if (VK_QUEUE_COMPUTE_BIT & queueFamilies[familyIndex].queueFlags)
            {
                mComputeQueue.reset(new GIDeviceQueueVk(weak_from_this(), queue, familyIndex));
            }

            if (VK_QUEUE_TRANSFER_BIT & queueFamilies[familyIndex].queueFlags)
            {
                mTransferQueue.reset(new GIDeviceQueueVk(weak_from_this(), queue, familyIndex));
            }
        }
    }
    ...
    SharedPtr<GIDeviceVk> GIDeviceBuilderVk::Build()
    {
        ...
        auto raw = new GIDeviceVk(
            mInstance,
            mPhysicalDevice,
            mCreateInfo);
        return SharedPtr<GIDeviceVk>(raw);
    }
}