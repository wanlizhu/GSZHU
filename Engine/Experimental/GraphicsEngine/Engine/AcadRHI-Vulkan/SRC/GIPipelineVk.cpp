...
namespace AutoCAD::Graphics::Engine
{
    ...
    */
    ...
    {
        assert(path.extension().wstring() == L".spv");
        if (mShaderReflection == nullptr)
        {
            mShaderReflection = SharedPtr<GIShaderReflectionVk>(new GIShaderReflectionVk());
        }
        mShaderReflection->LoadFromJSON(path.extension().wstring() + L".json");
