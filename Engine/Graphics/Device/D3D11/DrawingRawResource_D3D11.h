...
namespace Engine
{
    ...
    public:
		...
		virtual void CopySubresource(std::vector<D3D11_SUBRESOURCE_DATA>& data) = 0;
    ...
    {
    public:
        DrawingRawTexture2D_D3D11(std::shared_ptr<DrawingDevice_D3D11> pDevice, const D3D11_TEXTURE2D_DESC& desc, const std::vector<D3D11_SUBRESOURCE_DATA>& data) : DrawingRawTexture_D3D11(pDevice, desc.Width), m_desc(desc)
        {
		...
		void CopySubresource(std::vector<D3D11_SUBRESOURCE_DATA>& data) override
		{
			ID3D11Texture2D* pUploadTextureRaw = nullptr;
			m_desc.Usage = D3D11_USAGE_STAGING;
			m_desc.CPUAccessFlags = D3D11_CPU_ACCESS_READ;
			m_desc.BindFlags = 0;

			HRESULT hr = m_pDevice->GetDevice()->CreateTexture2D(&m_desc, data.data(), &pUploadTextureRaw);
			assert(SUCCEEDED(hr));

			m_pDevice->GetDeviceContext()->CopyResource(m_pTexture2D.get(), pUploadTextureRaw);
		}
