...
void DrawingDevice_D3D11::UpdateTexture(std::shared_ptr<DrawingTexture> pTex, const void* pData[], uint32_t size[], uint32_t slices)
{
    std::vector<D3D11_SUBRESOURCE_DATA> subResData(slices);

    auto pTextureRaw = std::dynamic_pointer_cast<DrawingRawTexture_D3D11>(pTex->GetResource());
