...
void DrawingDevice_D3D12::SetIndexBuffer(std::shared_ptr<DrawingIndexBuffer> pIB)
{
    if (pIB != nullptr)
    {
        auto pIndexBuffersRaw = std::dynamic_pointer_cast<DrawingRawIndexBuffer_D3D12>(pIB->GetResource());
        auto pCommandList = m_pDirectCommandManager->GetCommandList();
...
void DrawingDevice_D3D12::UpdateTexture(std::shared_ptr<DrawingTexture> pTex, const void* pData[], uint32_t size[], uint32_t slices)
{
    std::vector<D3D12_SUBRESOURCE_DATA> subResData(slices);

    auto pTextureRaw = std::dynamic_pointer_cast<DrawingRawTexture_D3D12>(pTex->GetResource());
    ...
    pTextureRaw->CopySubresource(subResData);
}
