...
namespace Engine
{
    ...
    protected:
        ...
        static const uint32_t MAX_INDEX_COUNT = 65536;
        ...
        void reset(const uint32_t maxSize);
        void Flush(DrawingResourceTable& resTable, const uint32_t maxSize);
        ...
        void BindVertexInputs(DrawingPass& pass);
        ...
        void FlushData_Legacy(DrawingResourceTable& resTable, std::shared_ptr<std::string> pTextureName);

        void FillInstanceData(const void* src, const uint32_t size, const uint32_t maxSize, DrawingResourceTable& resTable, std::shared_ptr<std::string> pTextureName);

        virtual void UpdateConstants(DrawingResourceTable& resTable);
