...
void ForwardRenderer::DefineShaderResource(DrawingResourceTable& resTable)
{
	...
	case eDevice_D3D12:
		...
		break;
	case eDevice_Vulkan:
		DefineVertexShader(BasicPrimitiveVertexShader(), strPtr("Asset\\Shader\\SPIRV\\vert.spv"), strPtr("main"), resTable);
		DefinePixelShader(BasicPrimitivePixelShader(), strPtr("Asset\\Shader\\SPIRV\\frag.spv"), strPtr("main"), resTable);
		break;
	default:
