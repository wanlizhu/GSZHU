...
void BaseRenderer::reset(const uint32_t maxSize)
{
...
void BaseRenderer::FillInstanceData(const void* src, const uint32_t size, const uint32_t maxSize, DrawingResourceTable& resTable, std::shared_ptr<std::string> pTextureName)
{
    if (m_size + size > maxSize) 
        Flush(resTable, maxSize);
    memcpy((char*)m_pData + m_size, src, size);

    m_size += size;
    m_instanceCount++;
}

void BaseRenderer::Flush(DrawingResourceTable& resTable, const uint32_t maxSize)
{
    Draw(resTable);
    reset(maxSize);
}

void BaseRenderer::FlushData_Legacy(DrawingResourceTable& resTable, std::shared_ptr<std::string> pTextureName)
{
    auto pEntry = resTable.GetResourceEntry(pTextureName);
    assert(pEntry != nullptr);

    const void* pData[256] = { nullptr };
    uint32_t size[256] = { 0 };
    uint32_t slices = 1;
    ...
    m_pDevice->UpdateTexture(pTex, pData, size, 1);
}
...
void BaseRenderer::DefineNullInputVertexFormat(DrawingResourceTable& resTable)
{
...
void BaseRenderer::DefineScreenConstants(DrawingResourceTable& resTable)
{
...
void BaseRenderer::DefineLineSmoothingConstants(DrawingResourceTable& resTable)
{
    auto pDesc = std::make_shared<DrawingConstantBufferDesc>();
...
void BaseRenderer::BindVertexInputs(DrawingPass& pass)
{
...
void BaseRenderer::UpdateConstants(DrawingResourceTable& resTable)
{
    ...
    if (pParam != nullptr)
    {
        float width = (float)gpGlobal->GetConfiguration<AppConfiguration>().GetWidth();
        float height = (float)gpGlobal->GetConfiguration<AppConfiguration>().GetHeight();
