...
REQUIRE(FT64_EQUAL(m1[0][0], m2[0][0]));\
REQUIRE(FT64_EQUAL(m1[0][1], m2[0][1]));\
REQUIRE(FT64_EQUAL(m1[0][2], m2[0][2]));\
REQUIRE(FT64_EQUAL(m1[0][3], m2[0][3]));\
REQUIRE(FT64_EQUAL(m1[1][0], m2[1][0]));\
REQUIRE(FT64_EQUAL(m1[1][1], m2[1][1]));\
REQUIRE(FT64_EQUAL(m1[1][2], m2[1][2]));\
REQUIRE(FT64_EQUAL(m1[1][3], m2[1][3]));\
REQUIRE(FT64_EQUAL(m1[2][0], m2[2][0]));\
REQUIRE(FT64_EQUAL(m1[2][1], m2[2][1]));\
REQUIRE(FT64_EQUAL(m1[2][2], m2[2][2]));\
REQUIRE(FT64_EQUAL(m1[2][3], m2[2][3]));\
REQUIRE(FT64_EQUAL(m1[3][0], m2[3][0]));\
REQUIRE(FT64_EQUAL(m1[3][1], m2[3][1]));\
REQUIRE(FT64_EQUAL(m1[3][2], m2[3][2]));\
REQUIRE(FT64_EQUAL(m1[3][3], m2[3][3]));
...
TEST_CASE("Test vector of math library", "[Math/Vector]") {
    ...
    auto D = Dot(V, V);
    auto d = glm::dot(v, v);
    REQUIRE(FT64_EQUAL(D, d));

    auto V3 = Cross(V, V2);
    auto v3 = glm::cross(v, v2);
    REQUIRE(FT64_EQUAL(V3[0], v3[0]));
    REQUIRE(FT64_EQUAL(V3[1], v3[1]));
    REQUIRE(FT64_EQUAL(V3[2], v3[2]));

    auto L = Length(V);
    auto l = glm::length(v);
    REQUIRE(FT64_EQUAL(L, l));

    auto V4 = Normalize(V);
    auto v4 = glm::normalize(v);
    REQUIRE(FT64_EQUAL(V4[0], v4[0]));
    REQUIRE(FT64_EQUAL(V4[1], v4[1]));
    REQUIRE(FT64_EQUAL(V4[2], v4[2]));

    {
        auto Vx = -V;
        auto vx = -v;
        REQUIRE(FT64_EQUAL(Vx[0], vx[0]));
        REQUIRE(FT64_EQUAL(Vx[1], vx[1]));
        REQUIRE(FT64_EQUAL(Vx[2], vx[2]));
    }
...
TEST_CASE("Test matrix of math library", "[Math/Matrix]") {
    ...
    REQUIRE(FT64_EQUAL(V2[0], v2[0]));
    REQUIRE(FT64_EQUAL(V2[1], v2[1]));
    REQUIRE(FT64_EQUAL(V2[2], v2[2]));
